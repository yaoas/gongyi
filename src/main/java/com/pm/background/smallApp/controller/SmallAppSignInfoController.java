 ...
 */
@RestController
@RequestMapping("/smallApp/signInfo")
public class SmallAppSignInfoController extends BaseController {
    @Autowired
    ...
    public Object signUpActiveInfo(HttpServletRequest request,@RequestBody SignInfo signInfo) {
        if(signInfo.getOpenId()==null||"".equals(signInfo.getOpenId())){
            ...
            User user = userService.byAccountGetId(signInfo.getOpenId());
    ...
    public Object approvalList(HttpServletRequest request,SignInfo signInfo) {
        if(signInfo.getOpenId()==null||"".equals(signInfo.getOpenId())){
            R.fail("请先登录");
        }
        User user = userService.byAccountGetId(signInfo.getOpenId());
    ...
    public Object pageList(HttpServletRequest request,SignInfo signInfo) {
        if(signInfo.getOpenId()==null||"".equals(signInfo.getOpenId())){
            R.fail("请先登录");
        }
        User user = userService.byAccountGetId(signInfo.getOpenId());
        ...
        qw.eq("active_id", signInfo.getActiveId());
        IPage page = signInfoService.page(new Page(signInfo.getCurrentPage(), signInfo.getPageSize()), qw);
    ...
    public Object endSignInfo(@RequestBody SignInfo signInfos) {
        if(signInfos.getOpenId()==null||"".equals(signInfos.getOpenId())){
            R.fail("请先登录");
        }
     ...
     //   float startTimeHour = startTime.get(Calendar.HOUR);
       // float endTimeHour = endTime.get(Calendar.HOUR);
        ...
        User user = userService.byAccountGetId(signInfos.getOpenId());
        Long userId = user.getId();
