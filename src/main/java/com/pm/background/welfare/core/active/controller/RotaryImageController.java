 ...
 */
@RestController
@RequestMapping("/rotaryImage")
public class RotaryImageController extends BaseController {
    @Autowired
    ...
    public Object pageList(RotaryImage rotaryImage) {
        ...
        if(rotaryImage.getActiveName()!=null&&!"".equals(rotaryImage.getActiveName())){
            qw.like("activeName",rotaryImage.getActiveName());
        }
     ...
     */
    @Transactional
    @RequestMapping("/publishRotaryImage")
    public Object publishRotaryImage(@RequestBody RotaryImage rotaryImage) {
        if(rotaryImage.getActiveId()==null||"".equals(rotaryImage.getActiveId())){
            return R.fail("活动不能为空");
        }if(rotaryImage.getImageAddress()==null||"".equals(rotaryImage.getImageAddress())){
            return R.fail("图片不能为空");
        }
        if (rotaryImage.getActiveId() != null && !"".equals(rotaryImage.getActiveId())) {
            ActiveInfo activeInfo = iActiveInfoService.getById(rotaryImage.getActiveId());
            rotaryImage.setActiveName(activeInfo.getTitle());
        }
        String statusName = iDictService.byIdGetName(rotaryImage.getStatus());
        rotaryImage.setStatusName(statusName);

        boolean publishFlag = rotaryImageService.saveOrUpdate(rotaryImage);
        if (publishFlag) {
            return R.ok("添加成功！");
        } else {
            return R.fail("添加失败！");
