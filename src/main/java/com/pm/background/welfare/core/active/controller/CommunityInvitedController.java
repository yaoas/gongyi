 ...
 */
@RestController
@RequestMapping("/communityInvited")
public class CommunityInvitedController extends BaseController {
    @Autowired
    ...
    public Object pageList(CommunityInvited communityInvited) {
    ...
    public Object applyIntoCommunity(@RequestBody CommunityInvited communityInvited) {
        Long userId = ShiroUtils.getUserEntity().getId();
    ...
    public Object queryAddUser(Long communityId) {
        Long userId = ShiroUtils.getUserEntity().getId();
    ...
    public synchronized Object quitCommunity(@RequestBody CommunityInvited communityInvited) {
        Community communityUser = communityService.getById(communityInvited.getCommunityId());
        //删除 社区人员中间表
        Long userId = ShiroUtils.getUserEntity().getId();
       /* if(userId ==communityUser.getCreateBy()){
        ...
        community.setPeopleNumber(peopelNumber);
        community.setId(communityInvited.getCommunityId());
        int i = communityService.updatePeopleNumber(community);
        //}
        if (i > 0) {
            return R.ok("退出成功！");
        } else {
    ...
    public synchronized Object insertCommunityChat(@RequestBody CommunityChat communityChat) {
        Long userId = ShiroUtils.getUserEntity().getId();
        User user = userService.getById(userId);
        communityChat.setUserId(userId);
