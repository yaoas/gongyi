...
public class HttpClientUtil {
    ...
    public static void main(String[] args) throws Exception{
        String tempdata=HttpGet("http://api.kemengjizhang.com/v3.0/komoi/mygoods/list");
      ...
      /*  for (int i = 0; i < getarray.length(); i++) {            //遍历数组
            JSONObject sonObject = getarray.getJSONObject(i);    //获得数组中的第一个对
            System.out.println(sonObject.get("id"));            //根据字段名获得对象的每一个属性字段
            System.out.println(sonObject.get("title"));
            System.out.println(sonObject.get("detail"));
        }
*/
       List<ButtShop> list =  com.alibaba.fastjson.JSONObject.parseArray(getarray.toString(), ButtShop.class);

       for(ButtShop test : list){
